    ...
    describe('#Get User'.cyan, function(done) {
        it('should return user without password and salt when calling with safeJSON', function(done) {
            User.hashPassword(values.password, function(password, salt) {
                User.create({'email' : values.email, 'password': password, 'password_salt': salt}, function(err, user) {
                    if (err) throw err;
    ...
    describe('#Check User credentials'.cyan, function() {
        ...
        before(function(done) {
            User.hashPassword(newValues.password, function(password, salt) {
                User.create({'email' : newValues.email, 'password': password, 'password_salt': salt}, function(err, user) {
                    if (err) throw err;
