...
var UserModel = function(resourceful) {
    ...
    User.string('email', {format: 'email', required: true, unique: true});
    User.string('password_salt', {restricted: true});
    User.string('password', {required: true, restricted: true, minLength: 7});
    /**
...
//             
    ...
    User.method('hashPassword', function(password, callback) {
        ...
        if (callback) callback(hash, salt);
    ...
    User.method('checkCredentials', function(email, password, callback) {
        ...
        this.find({'email': email}, function(err, result) {
            if (result[0].password !== encryptPassword(password, result[0].password_salt)) {
                result = [];    
            }
